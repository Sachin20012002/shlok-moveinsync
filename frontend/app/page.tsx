      ...
      <main className={styles.main}>
        ...
        <section className={styles.workspace}>
          <div className={styles.incidentPane}>
            <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>PRIORITY QUEUE</p><h2>Incidents</h2></div><span>{dashboard.activeIncidentCount} open</span></div>
            {loading ? <div className={styles.empty}>Loading operations data...</div> : incidents.length === 0 ? <div className={styles.empty}><Check size={24} /><strong>No active signals</strong><p>Upload the sample CSV to run the first evaluation.</p></div> : (
              <div className={styles.incidentList}>{incidents.map((incident) => <button key={incident.id} className={selectedIncident?.id === incident.id ? styles.incidentSelected : styles.incidentItem} onClick={() => setSelectedId(incident.id)}><span className={`${styles.severity} ${styles[incident.severity]}`}>{incident.severity}</span><strong>{incident.title}</strong><small>{incident.currentValue}% against {incident.slaValue}% SLA</small><ChevronRight size={18} /></button>)}</div>
            )}
          </div>
          ...
          <div className={styles.detailPane}>
            {!selectedIncident ? <div className={styles.empty}><Activity size={24} /><strong>Evidence appears here</strong><p>Select an incident after evaluating a dataset.</p></div> : (
              <>
                ...
                <button className={styles.acknowledge} onClick={() => void handleAcknowledge()} disabled={selectedIncident.status !== "open"}><Check size={18} /> {selectedIncident.status === "open" ? "Acknowledge incident" : "Incident acknowledged"}</button>
              </>
            )}
          </div>
        </section>
