      ...
      <main className={styles.main}>
        <header className={styles.header}>
          ...
          <div className={styles.headerActions}>
            ...
            <label className={`${styles.uploadButton} ${uploading ? styles.buttonBusy : ""}`}><Upload size={18} /> {uploading ? "Processing..." : "Upload trip CSV"}<input type="file" accept=".csv,text/csv" onChange={handleUpload} disabled={uploading} /></label>
          </div>
        </header>
        ...
        {uploadResult && <div className={styles.uploadResult} role="status"><Check size={18} /><span><strong>{uploadResult.filename}</strong> processed: {uploadResult.validRows} valid, {uploadResult.invalidRows} invalid, {uploadResult.skippedRows} skipped.</span></div>}
        ...
        <section className={styles.workspace}>
          ...
          <div className={styles.detailPane}>
            {!selectedIncident ? <div className={styles.empty}><Activity size={24} /><strong>{loading ? "Loading evidence" : "No incident selected"}</strong><p>{loading ? "Trip evidence will appear when the workspace is ready." : "Choose a priority incident to review its evidence."}</p></div> : (
              <>
